...
var uix = {
     ...
     * @param {boolean|string|Object|function} [show] Indica si mostrar u ocultar el elemento. Si no se especifica se
     * visualiza u oculta según su estado actual. Como alternativa a este parámetro se puede especificar aquí el nombre
     * de la transición, las opciones de la misma o la función que se llamará al finalizar la transición.
     * @param {string|Object|function} [transition] Nombre de transición, opciones de la misma o función que se llamará
     * al finalizar la transición.
     * @param {Object|function} [options] Opciones de la transición o función que se llamará al finalizar la transición.
     ...
     * @param {function} [options.start] Función que se llama al iniciar transición.
     * @param {function} [options.done] Función que se llama al finalizar la transición.
     */
    toggle: function(el, show, transition, options) {
        ...
        // Se comprueba si el elemento está oculto, la alternancia y si no es necesario hacer el cambio
        var hidden = uix.hasClass(el, 'ui-hide');
        if (show == null) {
            show = hidden;
        } else if (show && !hidden || !show && hidden) {
            // Aunque no sea necesario realizar la transición se notifica el inicio y final de la transición
            if (options.start) {
                options.start();
            }
            if (options.done) {
        ...
        // Se comprueba si hay una transición en proceso para cancelarla inmediatamente

        /**
         * Limpia el estado de la transición.
         * @param {Element} el Elemento especificado.
         * @param {function} toggleEndListener Manejador del evento 'transitionend' añadido anteriormente.
         * @param {string} transitionClassNames Nombres de clases de transición añadidas.
         */
        var clear = function(el, toggleEndListener, transitionClassNames) {
            // Opción A) Control de fin de transición por temporizador:
            // clearTimeout(el.dataset.toggleTimeout);
            // delete el.dataset.toggleTimeout;
            // Opción B) Control de fin de transición por evento 'transitionend':
            ...
            uix.removeClass(el, transitionClassNames);
            delete el.dataset.transitionClassNames;
        ...
        if (toggleEndListener) {
            clear(el, toggleEndListener, el.dataset.transitionClassNames);
        }

        // Se realiza la transición
        if (transition) {
            // Se añade las clases de transición
            var transitionClassNames = 'ui-transition-' + transition;
            if (options.reverse) {
                transitionClassNames += ' ui-reverse';
            }
            el.style.transition = 'none'; // -> Se añade para evitar la transición inicialmente
            uix.addClass(el, el.dataset.transitionClassNames = transitionClassNames);

            // Se fuerza el REFLOW del elemento
            el.offsetHeight + el.offsetWidth;

            // Se controla el final de la transición
            toggleEndListener = function() {
                // Opcion B)
                if (event.target !== el) {
                    return;
                }
                clear(el, toggleEndListener, transitionClassNames);
            ...
            // Opción A)
            // el.dataset.toggleTimeout = setTimeout(end, uix.getTransitionDuration(el));
            // Opción B)
            el.addEventListener('transitionend', uix.putState(el, 'toggleEndListener', toggleEndListener));
    ...
    putState: function(el, name, value) {
        var state = null, id;
        if (id = el.dataset.uixStateId) {
            state = this._stateMap[id];
        } else {
            id = this._stateNextId++;
        }
        if (!state) {
            state = this._stateMap[id] = {};
        }
    ...
    getState: function(el, name) {
        var id, state;
        if ((id = el.dataset.uixStateId) && (state = this._stateMap[id])) {
    ...
    toggleLoader: function(show, context) {
        ...
        if (loader) {
            ...
            //     this.toggleLoaderTimeout = 0;
            // }
            // this.toggleLoaderTimeout = setTimeout(function() {
            uix.toggle(loader, show, 'fade');
            // }, 50);
            // NOTE: Descomentar para evitar mostrar el indicador de carga durante unos instances en secuencias rápidas
            // de visualización y ocultación.
        }
