...
// }}
    ...
    openDialog: function(name, options) {
        ...
        var container = options.container || document.body,
            wrapper = uix.child(container, '.uix-dialog-wrapper');
        if (!wrapper) {
            container.appendChild( wrapper = uix.create('div', {
                'class': 'uix-dialog-wrapper uix-hide --display-none'
            }));
        }
        // Capa de ocultación
        var overlay = wrapper.querySelector('.uix-overlay');
        if (!overlay) {
        ...
        return dialog.include(wrapper, options, function(err, view) {
            ...
            var curr = uix.getState(wrapper, 'current');
            // Se actualiza la referencia al diálogo actual en el contenedor
            uix.putState(wrapper, 'current', dialog);
            ...
            if (options.fragment) {
                ...
                if (options.replace) {
                    uix.history.replace(path);
                } else {
                    uix.history.push(path);
                }
                // Evento de seguimiento
                uix.track('open_view', view);
            }
            // Control de cierre frente a cambios de estado o URL
            if (!uix.hasState(wrapper, 'historychange')) {
                uix.history.on('historychange', uix.putState(wrapper, 'historychange', function() {
                    var curr = uix.getState(wrapper, 'current');
    ...
    closeDialog: function(view, callback) {
        var wrapper = view.root.parentElement,
            overlay = wrapper.querySelector('.uix-overlay');
        if (view === uix.getState(wrapper, 'current')) {
            uix.clearState(wrapper, 'current');
            if (uix.hasState(wrapper, 'historychange')) {
                uix.history.off('historychange', uix.clearState(wrapper, 'historychange'));
            }
            if (overlay) {
        ...
        view.toggle(false, {
                ...
                if (!uix.hasState(wrapper, 'current')) {
                    uix.toggle(wrapper, false);
                }
                view.remove();
                if (callback) {
                    callback();
