...
var uix = {
        ...
        if (content) {
            if (uix.isArrayLike(content)) {
                for (var i = 0; i < content.length; i++) {
                    el.appendChild(Node.prototype.isPrototypeOf(content[i]) ? content[i] :
                        document.createTextNode(content[i]));
                }
            } else if (Node.prototype.isPrototypeOf(content)) {
    ...
    isArrayLike: function(obj) {
        return Array.isArray(obj) ||
            NodeList.prototype.isPrototypeOf(obj) || HTMLCollection.prototype.isPrototypeOf(obj);
    ...
    hideToast: function(name) {
        var el = Element.isPrototypeOf(name) ? name :
            document.querySelector('body > .uix-toast' + (name ? '[data-name="' + name + '"]' : ''));
        if (el) {
            if (el.dataset.timer) {
