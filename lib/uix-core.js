...
var uix = {
    ...
    load: function(url, options, complete) {
        ...
        if (options.data && options.method === 'GET') {
            ...
            parts.query = URL.formatQuery(parts.query ? Object.extend(URL.parseQuery(parts.query), options.data) : options.data);
        ...
        // Timeout por defecto (12.5 sg)
        xhr.timeout = (options.timeout === undefined) ? 12500 : options.timeout;
     ...
     * @param {string|HTMLCollection|NodeList|Node[]} [content] Contenido del elemento.
     * @param {object} [events] Eventos a añadir.
...
// }}
        ...
        var hidden = uix.hasClass(el, 'uix-hide');
        if (show == null) {
            show = hidden;
        ...
        var clearState = function() {
            // Recuperamos las clases definidas antes de iniciar la transición
            var classNames = uix.getState(el, 'toggleClassNames');
            if (classNames) {
                el.className = classNames.join(' ');
                uix.clearState(el, 'toggleClassNames');
            }
        ...
        // Se comprueba si se ha especificado transición, via opciones o por clase de estilo (uix-transition-)
        var transitionClass = uix.getClassEnding(el, 'uix-transition-'),
            transition = options.transition || transitionClass;

        if (transition && transition !== 'none') {
            // Obtenemos las clases definidas actualmente en el elemento
            ...
            if (transition !== transitionClass) {
                if (transitionClass) {
                    uix.removeClass(el, 'uix-transition-' + transitionClass);
                }
                uix.addClass(el, 'uix-transition-' + transition);
            }
            // Se comprueba si invertir la transición
            if (!!options.reverse != uix.hasClass(el, 'uix-reverse')) {
                ...
                // manualmente la clase "uix-reverse".
            }
            ...
            // Se añade o elimina la clase "uix-hide"
            uix.toggleClass(el, 'uix-hide', !show);
            uix.putState(el, 'toggleClassNames', (function() {
                if (show) {
                    classNames.splice(classNames.indexOf('uix-hide'), 1);
                } else {
                    classNames.push('uix-hide');
                }
                return classNames;
            })());
...
// }}
            ...
            var ts = el.style.transition;
            if (transitionClass) {
                // Si hay clase de transición se anula añadiendo la propiedad "transition" a "none"
                el.style.transition = 'none';
            } else {
                // BUGFIX: Evita problemas de visualización como en Chrome 62.0.3202.73 en donde no se acaban de
                // visualizar elementos recién añadidos al DOM. Parece que de esta forma se fuerza el pintado del
                // elemento.
                el.style.transitionDuration = window.getComputedStyle(el, null).getPropertyValue('transition-duration')
                    || '0s';
            }
            // Se comprueba la propiedad display
            if (show && el.style.display === 'none') {
            ...
            // Se añade o elimina la clase "uix-hide"
            uix.toggleClass(el, 'uix-hide', !show);
            // Se restaura el valor previo de la propiedad transición.
    ...
    toggle: function(el, show, options) {
        ...
        // Se comprueba si se ha definido o especificado transición.
        var trans = uix.getClassEnding(el, 'uix-transition-');
        if (options.transition && options.transition !== trans) {
            uix.removeClass(el, 'uix-transition-' + trans);
            if (options.transition !== 'none') {
                uix.addClass(el, 'uix-transition-' + (trans = options.transition));
            } else {
                trans = null;
            }
        }
        // NOTE: La inversión de la transición solo se puede especificar programáticamente por opciones, en otros casos
        // se eliminará la clase si está añadida.
        uix.toggleClass(el, 'uix-reverse', options.reverse);

        // Igualmente la duración solo se puede modificar programáticamente.
        if (options.duration) {
            el.style.transitionDuration = options.duration + 's';
        } else {
        ...
        // Si se ha definido o especificado transición comprobamos si realmente esta se va a producir para la propiedad
        // "visibility" y por tanto podemos esperar el evento "transitionend" o no.
        if (trans) {
            var cs = window.getComputedStyle(el, null);
            // En primer lugar comprobamos que la propiedad "visibility" esté dentro de la transición y en segundo que
            // se vaya a producir cambio de valor, de "visible" a "hidden" o de "hidden" a "visible", condición para que
            // recibamos el evento "transitionend".
        ...
        uix.toggleClass(el, 'uix-hide', !show);
        if (trans) {
            // Si preveemos que se va a producir la transición sobre la propiedad "visibility" añadimos el tratamiento
            // del evento "transitionend".
            ...
            // En cualquier otro caso damos por finalizada la transición.
            done();
        }
    },
     ...
     * @param {Object} [options] Opciones adicionales.
     * @param {Element} [options.container] Elemento contenedor donde añadir el diálogo. Debe incluir la clase
     * "uix-dialog-container".
     * @param {string} [options.fragment] Indica la cadena a incluir como fragmento en la URL.
     * @param {boolean} [options.replace=false] Indica si reemplazar la ruta actual en lugar de añadir una nueva entrada
     * en el histórico de navegación cuando se especifique un fragmento que añadir a la URL.
    ...
    openDialog: function(name, options) {
        ...
        if (!container) {
            container = document.querySelector('.uix-dialog-container');
            if (!container) {
                container = uix.create('div', {
                    'class': 'uix-dialog-container uix-hide --display-none'
                });
        ...
        if (!overlay) {
            overlay = uix.create('div', {
                'class': 'uix-overlay uix-transition-fade uix-hide --display-none'
            });
        ...
        // Se crea e incluye el nuevo diálogo
        var dialog = View.create(name);
        // NOTE: Añadimos el estilo display a "none" para evitar que al incluir la vista descoloque al diálogo actual
        // por la disposición flexible (flex).
        Object.extend(true, options, { style: { display: 'none' } });
        // Se incluye el diálogo
        return dialog.include(container, options, function(err, view) {
            if (err) {
                console.log('Unable to open dialog: ' + name);
                return;
            }
            // Se comprueba si se ha incluido el botón de cierre para añadirle el tratamiento de cierre
            ...
            // Se comprueba si hay algún diálogo abierto en el mismo contenedor
            var curr = uix.getState(container, 'current');
            // Se actualiza la referencia al diálogo actual en el contenedor
            uix.putState(container, 'current', dialog);
            // Se visualiza el elemento contenedor y la capa de ocultación
            uix.toggle(container, true);
            if (overlay) {
                uix.toggle(overlay, true);
            }
            // Se visualiza el diálogo
            if (curr) {
                // Si hay un diálogo anterior se cierra previamente.
                uix.closeDialog(curr, function() {
                    view.toggle(true, options);
                });
            } else {
                view.toggle(true, options);
            }
            // Si se ha especificado un fragmento se añade a la ruta actual y se añade el tratamiento
            if (options.fragment) {
                var path = location.pathname + location.search + '#' + options.fragment;
                if (options.replace) {
                ...
                uix.track('open_view', view);
            }
            // Control de cierre frente a cambios de URL
            if (!uix.hasState(container, 'statechange')) {
                window.addEventListener('statechange', uix.putState(container, 'statechange', function() {
                    var curr = uix.getState(container, 'current');
                    if (curr) { // && curr.location !== window.location.href -> NOTE: No es necesario comprobar la ruta.
                        uix.closeDialog(curr);
                    }
                }), false);
            }
        });
     ...
     */
    closeDialog: function(view, callback) {
        var container = uix.closest(view.root, '.uix-dialog-container'),
            overlay = container.querySelector('.uix-overlay');
        if (view === uix.getState(container, 'current')) {
            uix.clearState(container, 'current');
            if (uix.hasState(container, 'statechange')) {
                window.removeEventListener('statechange', uix.clearState(container, 'statechange'));
            }
            if (overlay) {
        ...
        view.toggle(false, {
                ...
                if (!uix.hasState(container, 'current')) {
                    uix.toggle(container, false);
                }
                view.remove();
                if (callback) {
                    callback();
    ...
    // -----------------------------------------------------------------------------------------------------------------
    /**
     * Muestra un mensaje de notificación de error en la parte inferior de pantalla.
     * @param {string} message Mensaje a mostrar.
     */
    /* TO-DO: Especificar el tipo de mensaje, posibles cambios de icono y estilo, así como otras opciones como el tiempo
        o la clave de identificación para reemplazar el mensaje. */
    showToast: function(message) {
        var toast;
        uix.hideToast();
        document.body.appendChild(toast = uix.create('div', {
            'class': 'uix-toast uix-transition-slide-up uix-hide --display-none'
        }, [
            uix.create('span', {
            ...
            uix.create('a', {
                'class': 'uix-button icon icon-close'
            }, null, {
                'tap': function() {
                    uix.hideToast(toast);
                }
    ...
    /**
     * Oculta el mensaje de notificación de error especificado.
     * @param {Element} [toast] Elemento especificado.
     */
    hideToast: function(toast) {
        if (!toast && !(toast = document.querySelector('body > .uix-toast'))) {
            return;
        }
        uix.toggle(toast, false, {
            remove: true
        });
...
uix.forms = {
    ...
    error: function(target, message, focus) {
        if (uix.matches(target, '.uix-form')) {
            var box = uix.lastChild(target, '.uix-error-box');
            if (box) {
                if (message) {
                    box.querySelector('.text').innerHTML = message;
                    uix.toggle(box, true);
                } else {
                    uix.toggle(box, false);
                }
            } else if (message) {
                target.appendChild(box = uix.create('div', {
                    'class': 'uix-error-box uix-transition-slide-up uix-hide --display-none'
                }, [
                    uix.create('span', {
                        'class': 'icon icon-info'
                    }),
