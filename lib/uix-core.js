...
log('options: ' + JSON.stringify(options));
        ...
        var hidden = uix.hasClass(el, 'ui-hide') && el.dataset.toggling === undefined || el.dataset.toggling === 'hide';
log('hidden: ' + hidden);
        if (show == null) {
            show = hidden;
         ...
         */
        var clear = function(el, toggleEndListener) {
log('clearing...');
            el.removeEventListener('transitionend', toggleEndListener);
            uix.clearState(el, 'toggleEndListener');
            // Si se llamó a windows.requestAnimationFrame() se cancela la petición
...
log((show ? 'Removing' : 'Adding') + ' "ui-hide"...');
    ...
    toggleLoader: function(show, options) {
        console.log('toggleLoader(' + show + ', ' + JSON.stringify(options) + ')');
        ...
        if (options) {
            ...
            if (options.inmediate === undefined) {
                options.inmediate = defaults.inmediate;
            }
        } else {
            options = defaults;
        }
        ...
        if (loader) {
            uix.toggle(loader, show, 'fade', options);
        }
