...
var uix = {
    ...
    toggle: function(el, show, transition, options) {
        ...
        //     ...
        // }}
        // Opción B) Control de fin de transición por evento 'transitionend': ¿Nos podemos fiar al 100%? {{
        if (el.toggleListener) {
            el.removeEventListener('transitionend', el.toggleListener);
            el.toggleListener = null;
            // }}
        ...
        if (transition) {
            ...
            var transitionClass = 'ui-transition-' + transition;
            if (options.reverse) {
                transitionClass += ' ui-reverse';
            }
            el.style.transition = 'none'; // -> Se añade para evitar la transición inicialmente
            uix.addClass(el, transitionClass);
            ...
            var end = function() {
                ...
                uix.removeClass(el, transitionClass);
                el.style.transitionDuration = '';
            ...
            // }}
            // Opción B) {{
            el.addEventListener('transitionend', el.toggleListener = end);
            // }}
        } else {
