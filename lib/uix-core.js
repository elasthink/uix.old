...
uix.forms = {
    ...
    error: function(field, message) {
        var msg = uix.nextSibling(field, '.uix-error-msg');
        if (msg) {
            msg.innerHTML = message;
        } else {
            field.insertAdjacentHTML('afterend', '<span class="uix-error-msg">' + message + '</span>');
        }
        uix.addClass(field, 'uix-error');
    },
    ...
    clean: function(context) {
        ...
        for (var i = 0; i < errors.length; i++) {
            var messages = errors[i].parentNode.querySelectorAll('.uix-error-msg');
