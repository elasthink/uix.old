...
// }}
    ...
    showMessageDialog: function(options, context) {
        ...
        var overlay = uix.child(context, '.uix-overlay');
        if (!overlay) {
            // Si no existe se crea
            overlay = doc.createElement('div');
            overlay.setAttribute('class', 'uix-overlay hide');
        ...
        var styleClass = 'uix-dialog uix-dialog-' + options.type;
        ...
        if (options.icon) {
            ...
            icon.setAttribute('class', 'uix-dialog-icon icon-' + options.icon);
            dialog.appendChild(icon);

            // Cuerpo
            body = doc.createElement('div');
            body.setAttribute('class', 'uix-dialog-body');
        ...
        if (typeof(options.actions) === 'object') {
            ...
            actions.setAttribute('class', 'uix-actions');

            var add = function(text, handler, styleClass) {
                var button = doc.createElement('button');
                button.setAttribute('class', 'uix-button' + (styleClass ? ' ' + styleClass : ''));
        ...
        wrapper.setAttribute('class', 'uix-dialog-wrapper');
...
uix.forms = {
    ...
    /**
     * Inicializa las reglas para la validación de campos del formulario especificado.
     * @param {Element} form Formulario o elemento contenedor especificado.
     * @param {Object.<string, function>} rules Reglas a aplicar para la validación de los campos del formulario.
     */
    init: function(form, rules) {
        uix.putState(form, 'formRules', rules);
        // Validación individual frente a cambios
        var inputs = form.querySelectorAll(Object.keys(rules).join(','));
     ...
     * @param {string} selector Selector para localizar el campo.
     * @param {function|Object} validate Función de validación u opciones, incluyendo la función de validación.
     * @param {Object} [options] Opciones adicionales, incluyendo la función de validación.
     * @param {boolean} [options.changes=true] Indica si validar los cambios automáticamente.
     * @param {function} [options.validate] Función de validación especificada.
     */
    rule: function(form, selector, validate, options) {
        var defaults = {
            changes: true
        };
    ...
    validate: function(form, data) {
        ...
        for (var selector in rules) {
            if (input ? uix.matches(el = input, selector) : (el = form.querySelector(selector))) {
                if (rules[selector](el, data) === false) {
                    if (errors++ === 0) {
                        el.focus();
                    }
                }
     ...
     * @param {Element} input Elemento especificado.
     * @param {string} [message] Texto del mensaje a mostrar.
     * @param {boolean} [focus=false] Indica si mandar el foco al elemento.
     */
    error: function(input, message, focus) {
        if (message) {
            var line = uix.closest(input, '.uix-line');
            var msg = (line) ? uix.lastChild(line, '.uix-error-msg') : uix.nextSibling(input, '.uix-error-msg');
            if (msg) {
