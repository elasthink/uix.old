...
log('clearing...');
            ...
            // Si se llamó a setTimeout() para iniciar la transición en modo inmediato, se cancela
            var toggleTimeout = uix.getState(el, 'toggleTimeout');
            if (toggleTimeout) {
                clearTimeout(toggleTimeout);
                uix.clearState(el, 'toggleTimeout');
            } else {
                // Si se llamó a requestAnimationFrame() se cancela igualmente
...
log((show ? 'Removing' : 'Adding') + ' "ui-hide"...');
            ...
            if (options.inmediate) {
                uix.putState(el, 'toggleTimeout', window.setTimeout(start, 500));
                // NOTE: Aquí en lugar de llamar directamete al start() esperamos un tiempo mínimo para evitar que se
                // muestre el indicador cuando no es necesario porque se oculta inmediatamente.
                // start();
            } else {
