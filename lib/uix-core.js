...
// }}
    ...
    openDialog: function(name, options) {
        ...
        return dialog.include(container, options, function(err, view) {
            ...
            if (options.fragment) {
                history.pushState(history.state, '', '#' + options.fragment); // URL.qualify('#' + options.fragment)
                if (!uix.hasState(container, 'statechange')) {
                    window.addEventListener('statechange', uix.putState(container, 'statechange', function () {
                        var curr = uix.getState(container, 'current');
                        if (curr) { //  && curr.location !== window.location.href
                            uix.closeDialog(curr);
                        }
                    }), false);
                }
            }
            // Se añade el tratamiento del cierre automático al cambiar la URL asociada al diálogo
            /*
            if (!uix.hasState(container, 'statechange')) {
                window.addEventListener('statechange', uix.putState(container, 'statechange', function () {
                    var curr = uix.getState(container, 'current');
