...
// }}
    ...
    // -----------------------------------------------------------------------------------------------------------------
    /**
     * Crea una nueva instancia de la vista de diálogo especificado, la añade a un contenedor y la la muestra.
     * @param {Object} options Opciones adicionales.
     * @param {boolean} [options.overlay=true] Indica si mostrar o no la capa de ocultación (uix-overlay).
    ...
    openDialog: function(name, options) {
        ...
        // Si hay algún diálogo abierto sobre el mismo contenedor se cierra inmediátamente
        var curr = uix.getState(container, 'current');
        if (curr) {
            uix.closeDialog(curr);
        }

        // Se crea el nuevo diálogo
        var dialog = uix.putState(container, 'current', View.create(name));
        if (options.location) {
        ...
        return dialog.include(container, options, function(err, view) {
            ...
            view.toggle(true);
            // Cierre automático al cambiar la URL
...
uix.forms = {
    ...
    /**
     * Como alternativa a la anterior función, añade una nueva regla o función de validación para el campo especificado
     * por medio de un selector y con la posibilidad de especificar opciones adicionales, como la que indica si validar
     * o no los cambios automáticamente.
            ...
            if (message) {
                ...
                if (msg) {
                    ...
                    if (line) {
                        ...
                        input.insertAdjacentHTML('afterend', code);
                    }
                }
            }
