...
function View(settings) {
     ...
     * @private
     */
    this.template = this.settings.template || null;
     ...
     * @private
     */
    this.parentView = this.settings.parentView || null;
     ...
     * @private
     */
    this.dataURL = this.settings.dataURL || null;
    ...
    if (includeQueue.length > 0) {
        for (var i = 0, c = includeQueue.length, include, anchor; i < includeQueue.length; i++) {
            ...
            include.view.include(anchor, Object.extend(include.options, {
                ...
                if (err) {
                    console.log('Unable to include view "' + include.name + '".');
                }
                if (include.complete) {
...
View.cache = {
    ...
    /**
     * Devuelve la instancia asignada al identificador especificado. También se le puede pasar la referencia a su
     * elemento raíz.
     * @param {string|Element} id Identificador único de la vista o elemento raíz de la misma de donde obtenerlo
     * (data-view-id).
     * @return {View} Vista obtenida o null si no se encuentra ninguna vista.
     */
    get: function(id) {
        if (typeof(id) !== 'string') {
            id = id.dataset.viewId;
        }
    ...
    /**
     * Elimina la vista especificada de la caché.
     * @param {View} view Vista especificada.
     */
    remove: function(view) {
        delete this.map[view.id];
    }
};

/**
 * Atajo de la función View.cache.get() (@see {@link View.cache.get}).
 */
View.get = function(id) {
    return View.cache.get(id);
};
...
View.prototype.init = function(settings) {
    ...
    // Ajuste automático de la posición de scroll
    var self = this;
    this.on('vhide', function() {

    });
    this.on('vready', function() {
...
View.prototype.include = function(target, options, complete) {
    ...
    var attach = function(load) {
        ...
        // TODO: Es necesario forzar un Reflow aquí ???
        if (load) {
    ...
    if (this.root) {
        ...
        this.load(options, function (err, data) {
            ...
            self.render(options, function (err, root) {
                ...
                self.root = root;
                root.dataset.viewId = self.id;
                if (options.hide) {
                    uix.toggle(root, false);
                }
                attach(true);
            });
        });
 ...
 * @see uix.toggle
 */
View.prototype.toggle = function(show, transition, options) {
    // Tratamiento de parámetros (@see uix.toggle)
    if (show != null) {
    ...
    var self = this;
    uix.toggle(this.root, show, transition, Object.extend({}, options, {
        done: function() {
