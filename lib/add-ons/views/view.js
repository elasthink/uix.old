...
View.render = function(view, template, options, complete) {
    ...
    var includes = [];
    var includeView = function(name, settings, options) {
        settings = settings || {};
        if (view) {
            settings.parentView = view;
        }
        var v = View.create(name, settings),
            id = 'include-view-' + v.uid;
        includes.push({
            id: id,
            name: name,
            view: v,
 ...
 * al final del mismo
 * @param {Object} [options.hide] Indica si ocultar el elemento en la inclusión (display: none).
