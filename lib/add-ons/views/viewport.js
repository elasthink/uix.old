 ...
 * @param {string} path Ruta especificada, puede incluir cadena de búsqueda o consulta y fragmento.
...
Viewport.prototype.open = function(path, options, complete) {
    ...
    // Se parsea la ruta especificada para extraer los parámetros y el hash si se han especificado
    var req = URL.parse(path);
    if (req.query) {
        options.query = req.query;
    }
    if (req.hash) {
        options.hash = req.hash;
    }
    ...
    var next = function(view, node) {
        ...
        if (typeof(view.open) === 'function') {
            view.open(node.rest, Object.extend({}, options), function(err) {
                if (err) {
        ...
        for (var i = 0, node; i < nodes.length; i++) {
            ...
            if (node.view) {
                ...
                if (view) {
                    ...
                    // Se carga la vista
                    this.loadView(node.view, options, function(err, view) {
                        if (err) {
