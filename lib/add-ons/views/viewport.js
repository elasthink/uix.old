...
Viewport.prototype.findPathNodes = function(path, routes, nodes) {
    ...
    for (var i = 0, r, m; i < routes.length; i++) {
        ...
        if (!r.regexp) {
            if (r.end === undefined) {
                r.end = !!r.view; // Antes: !(Array.isArray(r.routes) && r.routes.length > 0);
                // NOTE: Cambiamos el criterio por defecto. Antes evaluábamos si el nodo definida subrutas pero ahora
                // simplemente comprobamos si el nodo defina una vista o no.
                // Si queremos cambiar este aspecto, por ejemplo cuando delegamos en una vista el procesamiento del
                // resto de la ruta, basta con indicar que el nodo no es final añadiendo la propiedad: end = false, de
                // esta forma no se evalua la expresión regular hasta el final de la ruta, añadiendo "$";
            }
