...
function Viewport(container, options) {
    ...
    if (this.topLevel) {
        ...
        uix.history.on('historypop', function(event) {
            ...
            if (this.backRequest) {
                if (this.backRequest.path) {
                    ...
                    if (!this.comparePaths(path, this.backRequest.path, this.backRequest.options)) {
                        // Si no coincide con la extraída del histórico, se continua extrayendo entradas hasta alcanzar
                        // la ruta solicitada o vaciar la pila.
                        if (uix.history.length > 1) {
                            // Se elimina la vista cargada asociada a las ruta extraída del histórico no coincidente con
                            // la ruta especificada en la petición de vuelta atrás.
            ...
            // Si la ruta es exactamente igual a la actual, incluyendo los parámetros de consulta y el fragmento, y no
            // se ha solicitado la recarga, acaba el proceso.
            // NOTE: Con esto cubrimos el caso de volver atrás eliminando el fragmento de la URL.
            if (!options.reload & this.comparePaths(path, this.path, { ignoreQuery: false, ignoreFragment: false })) {
                return;
            }
            if (options.back) {
