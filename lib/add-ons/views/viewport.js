...
Viewport.prototype.open = function(path, options, complete) {
    ...
    /**
     * Llama a la función de retorno y finaliza el proceso de carga o apertura.
     * @param err
     */
    ...
    var error = function(err, view) {
        ...
        if (View.exists('error')) {
            self.loadView('error', Object.extend(options, {
                ...
                self.pushView(path, view); // TODO: Es necesario ???
                // Se realiza la transición
                swap(view, err);
            });
        } else {
 ...
 * @param {function} complete Función de retorno que se llamará al concluir la transición entre las vistas.
 */
Viewport.prototype.switchView2 = function(next, options, complete) {
    var trans = null;
