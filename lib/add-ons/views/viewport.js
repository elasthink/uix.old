...
/**
 * Añade la vista a la pila asociándola a la ruta especificada. Si la vista ya está añadida se mueve a la cima de la
 * pila actualizando la ruta asociada.
 * @param {string} path Ruta especificada.
...
Viewport.Request.prototype.pushView = function() {
    ...
    if (this.view !== this.viewport.current) {
        // Si no lo es se añade a la pila o se mueve a la cima de la misma
        ...
        if (this.options.transition !== undefined) {
            ...
            this.transition = curr ? (curr.transition || uix.getClassEnding(curr.root, 'ui-transition-') || null) : null;
        }
