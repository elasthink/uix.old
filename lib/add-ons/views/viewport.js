 ...
 * @param {string} [basePath] Ruta base especificada.
 * @author terangel
 * @class
 */
    ...
    if (this.topLevel) {
        ...
        window.addEventListener('popstate', function(event) {
            ...
            if (self.backRequest) {
                ...
                if (!self.backRequest.path || self.backRequest.path === window.location.pathname) {
                    // Se añaden las opciones incluidas
                    Object.extend(options, self.backRequest.options);
                } else {
                    if (window.history.length > 0) {
                        // Se eliminan las vistas cargadas en la pila asociadas a las ruta extraídas del histórico no
                        // coincidentes con la petición
                        var v = self.findView(window.location.pathname);
                        if (v) {
            ...
            if (!options.transition) {
                options.transition = (options.back) ? self.transition : state.transition;
            }
            if (options.back && state.transition) {
                self.transition = state.transition;
            }
            self.open(window.location.pathname, options);
        });
    }
};
...
Viewport.prototype.open = function(path, options, complete) {
    ...
    // Se comprueba la base
    if (this.basePath && path.startsWith(this.basePath)) {
        path = path.substr(this.basePath.length);
        console.log('[Viewport] path -> ' + path);
    }
    ...
    var swap = function(view, err) {
        ...
        // Se guarda la posición actual de scroll en la vista saliente
        if (curr) {
            curr.scroll = {
                top: self.container.scrollTop,
                left: self.container.scrollLeft
            };
        }
        // Se posiciona la vista entrante a la posición de scroll de forma fija durante la transición
        Object.extend(view.root.style, {
            position: 'fixed',
            top: (view.scroll) ? -view.scroll.top + 'px' : 0,
            left: (view.scroll) ? -view.scroll.left + 'px' : 0
        });
        // NOTE: Cuidado con sobreescribir estas propiedades en la transición !!!

        // Se realiza la transición entre las vistas
        console.log('Viewport.open > toggling view...');
        view.toggle(true, options.transition, {
            reverse: options.back,
            start: function() {
                console.log('Viewport.open > toggle.start');
                // Se oculta el indicador de carga
                uix.toggleLoader(false);
                // Se oculta la vista actual
                if (curr) {
                    curr.toggle(false, options.transition, {
                        reverse: !options.back
                    });
                }
            },
            done: function() {
                console.log('Viewport.open > toggle.done');
                // Se reajusta la posición del scroll
                Object.extend(view.root.style, {
                    position: '',
                    top: '',
                    left: ''
                });
                Object.extend(self.container, {
                    scrollTop: view.scroll ? view.scroll.top : 0,
                    scrollLeft: view.scroll ? view.scroll.left : 0
                });
