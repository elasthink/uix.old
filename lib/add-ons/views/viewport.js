...
function Viewport(container, options) {
    ...
    if (this.topLevel) {
        ...
        /**
         * Número de entrada en el histórico iniciales.
         * @type {number}
         */
        ...
        window.addEventListener('popstate', function(event) {
            ...
            // Se obtiene la ruta de la URL en el navegador incluyendo la cadena de búsqueda o consulta (query)
            var path = location.pathname + location.search; // + location.hash;
            // Se comprueba si hay una petición de vuelta atrás
            if (self.backRequest) {
                // Se comprueba si se ha especificado ruta y si coincide con la extraída, mientras haya entradas en el
                // histórico de navegación
                if (self.backRequest.path && self.backRequest.path !== path && window.history.length - 1 > self.historyLength) {
                ...
                // Se sobrescribe la ruta cuando corresponda
...
Viewport.prototype.findPathNodes = function(path, routes, nodes) {
    ...
    for (var i = 0, r, m, sub; i < routes.length; i++) {
        r = routes[i];
        if (!r.regexp) {
        ...
        if (m) {
            ...
            if (!r.end) {
                // Si el nodo no es final se extrae la parte de la ruta que queda por procesar
...
Viewport.Request.prototype.pushView = function() {
    ...
    this.viewport.referer = this.options.referer || this.viewport.path;
    this.viewport.path = this.path;
    // TODO: Que pasa con los viewport que no son top-level?, podríamos usar en ese caso la ruta parcial?
