    ...
    if (this.topLevel) {
        ...
        window.addEventListener('popstate', function(event) {
            ...
            if (!options.transition) {
                options.transition = self.transition;
            }
            if (event.state && 'transition' in event.state) {
...
Viewport.prototype.open = function(path, options, complete) {
    ...
    /**
     * Actualiza la ruta actual, guarda la entrada en el histórico cuando proceda y finaliza el proceso llamando a la
     * función de retorno.
    ...
    var push = function(err) {
        ...
        if (self.topLevel) {
            if (options.history !== View.History.NONE) {
     ...
     * @param {View} next Vista siguiente recién cargada.
     * @param {Object} [err] Error especificado.
     */
    var swap = function(next, err) {
        // Se oculta el indicador de carga
        uix.toggleLoader(false);

        // Se comprueba si la vista es diferente a la actual
        if (next !== self.current) {
            // Se realiza la transición entre las vistas y se evalúa si eliminar instancias de las vistas del mismo tipo
            self.switchView(next, options, function() {
                push(err);
            });
        } else {
            push(err);
    ...
    var next = function(view, rest) {
        if (rest !== undefined && typeof(view.open) === 'function') {
            // TODO: Revisar y probar la implementación del tratamiento delegado de rutas parciales por vistas ya que hemos realizado muchos cambios y puede ser que no funcione correctamente
...
Viewport.prototype.switchView = function(next, options, complete) {
    var trans = '';
    if (this.transition = options.transition) {
        trans = View.Transition.parse(options.transition);
    }

    var curr = this.current;
    if (curr) {
        if (trans) {
            uix.addClass(curr.root, trans.getClass());
            if (!options.back) {
                uix.addClass(curr.root, '--reverse');
            }
        }
        uix.addClass(curr.root, '--hide');
    }

    if (trans) {
        // next.root.offsetHeight + next.root.offsetWidth; // -> REFLOW!
        uix.addClass(next.root, trans.getClass());
        if (options.back) {
            uix.addClass(next.root, '--reverse');
        }
    }
    uix.removeClass(next.root, '--hide');
    ...
    var self = this,
            ...
            if (curr) {
                curr.fire('vhide');
                curr.remove();
                View.Transition.clear(curr.root);
                if (options.back) {
            ...
            next.fire('vshow');
            View.Transition.clear(next.root);
    ...
    if (trans) {
        // this.switchTimer =
        setTimeout(done, trans.duration * 1000);
    } else {
        done();
    }
};
 ...
 * @type {Object.<string, View.Transition>}
 */
View.Transition.types = {};

/**
 * Añade un nuevo tipo de transición.
 * @param {string} name Nombre de la transición.
 * @param {Object} methods Implementación de métodos.
 */
View.Transition.add = function(name, methods) {
    var T = function() {
        View.Transition.call(this);
    }
    T.prototype = Object.extend(Object.create(View.Transition.prototype), methods);
    T.prototype.constructor = T;
    T.prototype.name = name;
    View.Transition.types[name] = T;
};

/**
 * Parsea la cada especificada creando un nuevo objeto de transición.
 * @param {string} str Cadena especificada.
 * @returns {View.Transition}
 */
View.Transition.parse = function(str) {
    var trans = null,
        parts = str.split(':'),
        name = parts[0];
    if (name in View.Transition.types) {
        trans = new View.Transition.types[name]();
 ...
 */
View.Transition.clear = function(el) {
 ...
 * @type {number}
 */
View.Transition.prototype.duration = 0;

/**
 * Función de inicialización para tratamiento de los parámetros especificados en la cadena de definición.
 * @param {...*} Parámetros especificados.
 */
View.Transition.prototype.init = function() {};

/**
 * Compone la cadena de definición de clases de estilo correspondiente a la transición y parámetros especificados.
 * @returns {string}
 */
View.Transition.prototype.getClass = function() {
    return 'ui-trans ui-trans-' + this.name;
};
 ...
 * @class
 */
View.Transition.add('fade', {
    /** @see View.Transition.prototype.duration */
    duration: 1.25,
});
 ...
 * @class
 */
View.Transition.add('slide', {
    /** @see View.Transition.prototype.duration */
    duration: 1.25,
    /** @see View.Transition.prototype.init */
    init: function(direction) {
        this.direction = direction || 'left';
    },
    /** @see View.Transition.prototype.className */
    getClass: function() {
        return View.Transition.prototype.getClass.call(this) + '-' + this.direction;
    }
});
