...
function Viewport(container, options) {
    ...
    if (this.topLevel) {
        ...
        uix.history.on('historypop', function(event) {
            ...
            if (this.backRequest) {
                var stop = (this.backRequest.path) ? this.comparePaths(path, this.backRequest.path) : true;
                // NOTE: Se compara las rutas excluyendo el fragmento
                if (!stop) {
                    // NOTE: Si se ha especificado una ruta y no coincide con la extraída del histórico, se continua
            ...
            var stateData = event.detail && event.detail.state && event.detail.state.data || {};
            if (!options.transition) {
                options.transition = (options.back) ? this.backTransition : stateData.transition;
            }
            if (options.back && stateData.transition) {
                this.backTransition = stateData.transition;
            }
 ...
 * comparando la ruta por separado, los parámetros de búsqueda sin importar el orden e incluyendo el fragmento
 * opcionalmente.
 * @param {string} path1 Ruta primera.
 * @param {string} path2 Ruta segunda con la que comparar.
 * @param {boolean} compareFragments Indica si comparar el fragmento o no.
 * @return {boolean} Devuelve verdadero si son iguales o equivalentes.
 */
Viewport.prototype.comparePaths = function(path1, path2, compareFragments) {
    path1 = URL.parse(path1, true); path2 = URL.parse(path2, true);
    return (path1.path === path2.path && path1.query.equals(path2.query) &&
        (!compareFragments || path1.fragment === path2.fragment));
};
...
Viewport.Request.prototype.pushView = function() {
    ...
    if (this.viewport.topLevel && this.options.history !== View.History.NONE) {
        ...
        } else if (!this.viewport.comparePaths(this.path, location.href)) { // TODO: Deberíamos comparar fragmentos?
