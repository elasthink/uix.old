...
function Viewport(container, options) {
    ...
    if (this.topLevel) {
        ...
        uix.history.on('historypop', function(event) {
            ...
            // Si la ruta es exactamente igual a la actual, incluyendo el fragmento, acaba el proceso.
            // NOTE: Con esto cubrimos el caso de volver atrás eliminando el fragmento de la URL
            if (this.comparePaths(path, this.path, { ignoreQuery: false, ignoreFragment: false })) {
                return;
            }
            if (options.back) {
