...
/**
 * Indica que no se debe mantener ninguna instancia de la vista cargada.
 * @type {string}
 * @constant
 */
View.KEEP_NONE = 'none';
...
/**
 * Indica que se debe mantener una única instancia de la vista cargada.
 * @type {string}
 * @constant
 */
...
/**
 * Indica que se pueden mantener múltiples instancias de la vista cargadas.
 * @type {string}
 * @constant
 */
...
Viewport.prototype.open = function(path, options, complete) {
    ...
    var swap = function(view, err) {
        ...
        if (view !== self.current) {
            ...
            if (self.current) {
                self.clearViewInstances(self.current);
            }
     ...
     */
    // TODO: La podemos llamar 'follow' mejor?
    ...
    var error = function(err, view) {
        ...
        if (View.handlers['error'] || View.templates['error']) {
            self.loadView('error', Object.extend(params, {
                error: err
            }), function(err, view) {
                if (err) {
    ...
    if (nodes.length > 0) {
        ...
        for (var i = 0, params = {}; i < nodes.length; i++) {
            ...
            if (typeof(nodes[i].handler) === 'function') {
                nodes[i].handler(params, function(err) {
            ...
            if (nodes[i].view) {
                ...
                if (view) {
                    ...
                    if (options.reload) {
                        // Se recarga la vista si así se ha solicitado
                    ...
                    // Se carga la vista
                    this.loadView(nodes[i].view, params, function(err, view) {
                        if (err) {
 ...
 * @private
 */
Viewport.prototype.loadView = function(name, params, complete) {
    var view;
    try {
        view = View.create(name, params);
    } catch (err) {
        return complete(err);
    }
    ...
    view.include(this.container, params, function(err) {
        complete(err, view);
    });
};
...
/**
 * Elimina por completo la vista especificada, tanto de la pila como del documento. Si se trata de la vista actual, se
 * marca para su posterior eliminación cuando pase a segundo plano.
 * @param {View} view Vista especificada.
 * @private
 */
Viewport.prototype.destroyView = function(view) {
    if (view === this.current) {
        // Si la vista se está visualizando actualmente se marca para su posterior destrucción
        view.destroyOnToggle = true;
    } else {
        view.destroy();
    }
};
    ...
    for (var i = this.views.length - 1, c = 0; i >= 0; i--) {
        if (this.views[i].view instanceof view.constructor) {
            if (++c > keep && (!active || this.views[i].view !== view)) {
                this.views.splice(i, 1);
                this.destroyView(this.views[i].view);
            }
        }
    }
};
...
Viewport.prototype.switchView = function(next, complete) {
    if (this.current) {
        var self = this,
            prev = this.current;
        prev.toggle(false, function() {
            prev.remove();
            if (prev.destroyOnToggle) {
                prev.destroy();
            }
        });
    }
